...
if (failures.length === 0) {
  ...
  const normalizeText = (value) => value
    ...
    .replaceAll(/-\s+/g, "-")
    .trim();
  const visibleText = normalizeText(readableHtml.replaceAll(/<[^>]+>/g, " "));
  ...
  assert(html.includes('id="project"'), "/resume: missing personal project anchor");
  assert(html.includes('id="credentials"'), "/resume: missing credentials anchor");
  assert(html.includes("Summary"), "/resume: missing summary heading");
  assert(html.includes("Technical Skills"), "/resume: missing technical skills heading");
  assert(html.includes("Professional Experience"), "/resume: missing professional experience heading");
  assert(html.includes("Personal Project"), "/resume: missing personal project heading");
  assert(html.includes("Education &amp; Certifications"), "/resume: missing combined education and certifications heading");
  ...
  for (const phrase of [
    ...
    careerData.summary,
    ...careerData.experience.flatMap((entry) => [entry.role, entry.organization, ...entry.bullets.map(plainText)]),
    ...careerData.projects.flatMap((project) => [project.name, project.context, ...project.bullets.map(plainText)]),
    ...
    assert(visibleText.includes(normalizeText(plainText(phrase))), `/resume: career-data phrase missing: ${phrase}`);
  }
}
